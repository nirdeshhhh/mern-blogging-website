...
const handleBannerUpload = async (e) => {
  ...
  const handlePublishEvent = async () => {
    ...
    if (!title || title.length === 0) {
      toast.error("Write a blog title to publish it");
      return;
    }

    if (!textEditor) {
      toast.error("Editor not initialized yet");
      return;
    }
    ...
    if (textEditor.isReady) {
      try {
        ...
        if (data.blocks.length) {
          const firstParagraph = data.blocks.find(block => block.type === "paragraph")?.data?.text || "";
          const shortDescription = firstParagraph.replace(/<[^>]+>/g, "").slice(0, 200); 
        ...
        console.log("Editor Save Error:", err);
      }
    }
  };   
